...
# Script is in: super_agent/tool/mcp_servers/activate/
if [[ -z "${REPO_ROOT:-}" ]]; then
    ...
    if [[ ! -d "$REPO_ROOT/super_agent/tool/mcp_servers" ]]; then
        ...
        for i in {1..10}; do
            ...
            if [[ -d "$current_dir/super_agent/tool/mcp_servers" ]]; then
        ...
        if [[ "$found" != "true" ]]; then
            echo "Error: Could not find repository root (looking for super_agent/tool/mcp_servers/ directory)" >&2
...
if [[ ! -d "$REPO_ROOT/super_agent/tool/mcp_servers" ]]; then
    echo "Error: mcp_servers directory not found at: $REPO_ROOT/super_agent/tool/mcp_servers" >&2
...
servers=(
    "super_agent.tool.mcp_servers.browser_use_mcp_server|"
    "super_agent.tool.mcp_servers.python_server|"
    "super_agent.tool.mcp_servers.vision_mcp_server|"
    "super_agent.tool.mcp_servers.audio_mcp_server|"
    "super_agent.tool.mcp_servers.reasoning_mcp_server|"
    "super_agent.tool.mcp_servers.reading_mcp_server|--transport sse"
    "super_agent.tool.mcp_servers.searching_mcp_server|"
    "super_agent.tool.mcp_servers.wiki_mcp_server|"
)

TOOLING_DIR="$REPO_ROOT/super_agent/tool"
